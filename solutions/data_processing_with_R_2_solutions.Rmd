...
population <- read_csv("population_total.csv") %>%        # load the population data
  gather(-c(country,geo),                                 # tidy the data
         key=year, 
         value=pop, 
         na.rm=TRUE) %>%   
...
co2_pp <- co2 %>%
  left_join(population) %>%                               # join on (country, geo, year)
...
geo_8 <- countries[c("geo","eight_regions")]  # get the columns 
co2_pp <- left_join(co2_pp, geo_8, by="geo")    # join to co2_pp
head(co2_pp)
...
co2_pp %>%
  group_by(eight_regions, year) %>%
  summarise(total_kt=sum(kt),total_pop=sum(pop)) %>%
  mutate(t_pp=1000*total_kt/total_pop) %>%
  ggplot(aes(x=year, y=t_pp, color=eight_regions)) + 
    geom_line() + 
...
makefig <- function(g){
  co2_pp %>%
    ...
    mutate(year=factor(year)) %>%
    ggplot(aes(x=year,y=log(t_pp), fill=year)) + 
    geom_violin() +
    labs(y="log10(tonnes CO2 per person)")
  ggsave(str_c(g,".png"))
}
